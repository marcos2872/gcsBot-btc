# src/model_trainer.py (COM VALIDAÇÃO DE PERDA DE DADOS)
...
class ModelTrainer:
    def __init__(self):
        self.feature_names = [
            ...
            'adx', 'adx_pos', 'adx_neg', 'dxy_close_change', 'vix_close_change',
            'gold_close_change', 'tnx_close_change'
        ]

    def _prepare_features(self, df: pd.DataFrame) -> pd.DataFrame:
        logger.debug("Iniciando cálculo de features...")
        initial_rows = len(df)
        epsilon = 1e-10
        ...
        logger.debug(f"Preparação de features concluída. Linhas removidas na limpeza: {rows_dropped} ({drop_percentage:.2f}% dos dados deste ciclo).")
        # ######################################
    ...
    def train(self, data: pd.DataFrame, all_params: dict):
        ...
        model_params = {k: v for k, v in all_params.items() if k not in ['future_periods', 'profit_mult', 'stop_mult', 'profit_threshold', 'stop_loss_threshold', 'prediction_confidence']}
